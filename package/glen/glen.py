...
def event_detection(sentence_list, path_to_ckpt, bs_TI=64, bs_TC=64, bs_TR=4, k=10):
    if not os.path.exists(path_to_ckpt):
        raise FileNotFoundError(f"The checkpoint dictionary \'{path_to_ckpt}\' does not exist. Download checkpoints from https://drive.google.com/file/d/1UU1UVPpYypRh5dPUhQ8TreAJd-uoLEh7/view")
    if not os.path.exists(os.path.join(path_to_ckpt, 'trigger_identifier.bin')) or not os.path.exists(os.path.join(path_to_ckpt, 'type_classifier.bin')) or not os.path.exists(os.path.join(path_to_ckpt, 'type_ranking.bin')):
        raise FileNotFoundError(f"Make sure all three checkpoints (trigger_identifier.bin, type_ranking.bin, type_classifier.bin) are in the dictionary.")
